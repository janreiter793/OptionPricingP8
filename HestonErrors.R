...
obtainHestonMetrics <- function(N, file) {
  ...
  ErrorFunction <- function(params, minimize = T) {
    ...
    if(minimize) {
      ...
      c(RMSE = sqrt(err/nrow(data)),
        MAE = err_abs / nrow(data),
        MEAN = sum(data$open) / nrow(data)) %>% return
    }
  }
  ...
  return(list(ErrorFunction(res$par, minimize = F), transformed_parameters))
}
...
vals_A <- A_list %>% unlist %>% unname
vals_B <- B_list %>% unlist %>% unname
vals_params_A <- A_params %>% unlist %>% unname
vals_params_B <- B_params %>% unlist %>% unname
conc_A <- data.frame(MEAN = vals_A[1:5 * 3],
                     RMSE = vals_A[1:5 * 3 - 2],
                     MAE = vals_A[1:5 * 3 - 1],
                     sigma_0_sqrd = vals_params_A[1:5 * 5 - 4],
                     kappa = vals_params_A[1:5 * 5 - 3],
                     eta = vals_params_A[1:5 * 5 - 2],
                     theta = vals_params_A[1:5 * 5 - 1],
                     rho = vals_params_A[1:5 * 5])
conc_B <- data.frame(MEAN = vals_B[1:5 * 3],
                     RMSE = vals_B[1:5 * 3 - 2],
                     MAE = vals_B[1:5 * 3 - 1],
                     sigma_0_sqrd = vals_params_B[1:5 * 5 - 4],
                     kappa = vals_params_B[1:5 * 5 - 3],
                     eta = vals_params_B[1:5 * 5 - 2],
                     theta = vals_params_B[1:5 * 5 - 1],
                     rho = vals_params_B[1:5 * 5])

rownames(conc_A) <- rownames(conc_B) <- c("AMZN", "BRK-B", "GOOG", "GOOGL", "TSLA")
conc_A %<>%
  mutate(pRMSE = RMSE / MEAN * 100,
         pMAE  = MAE / MEAN * 100)
conc_B %<>%
  mutate(pRMSE = RMSE / MEAN * 100,
         pMAE  = MAE / MEAN * 100)
